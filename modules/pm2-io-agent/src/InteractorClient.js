...
module.exports = class InteractorDaemonizer {
    ...
    this.ping(conf, (err, online) => {
      ...
      if (!online || err) {
        return cb
          ? err
            ? cb(err)
            : cb(new Error('Interactor not launched'))
  ...
  static daemonize(cst, conf, cb) {
    ...
    const testEnv =
      process.env.TRAVIS || (process.env.NODE_ENV && process.env.NODE_ENV.match(/test/));
      ...
      env: Object.assign(
        {
          ...
          DEBUG:
            process.env.DEBUG ||
            'interactor:*,-interactor:axon,-interactor:websocket,-interactor:pm2:client,-interactor:push',
        },
        process.env,
      ),
    ...
    const timeout = setTimeout((_) => {
      printOut(
        `${chalk.yellow('[PM2.IO][WARNING]')} Not managed to connect to PM2 Plus, retrying in background.`,
      );
    ...
    child.once('message', (msg) => {
      ...
      if (msg.km_data && msg.km_data.error === true) {
        if (!process.env.PM2_SILENT) {
          ...
          console.log(
            chalk.cyan('[PM2.IO]') +
              ' Contact support contact@keymetrics.io and send us the error message',
          );
        }
        return cb(msg);
        ...
        if (!process.env.PM2_SILENT) {
          console.log(
            '%s You must upgrade your bucket in order to monitor more servers.',
            chalk.red('[PM2.IO]'),
          );
        }
        return cb(msg);
  ...
  static getOrSetConf(cst, infos, cb) {
    ...
    configuration.public_key =
      process.env.PM2_PUBLIC_KEY ||
      process.env.KEYMETRICS_PUBLIC ||
      infos.public_key ||
      confFS.public_key;
    configuration.secret_key =
      process.env.PM2_SECRET_KEY ||
      process.env.KEYMETRICS_SECRET ||
      infos.secret_key ||
      confFS.secret_key;
    configuration.machine_name =
      process.env.PM2_MACHINE_NAME ||
  ...
  static launchAndInteract(cst, opts, cb) {
    ...
    this.getOrSetConf(Object.assign(cst, constants), opts, (err, conf) => {
      ...
      if (!process.env.PM2_SILENT) {
        console.log(
          chalk.cyan('[PM2 I/O]') + ' Using: Public key: %s | Private key: %s | Machine name: %s',
          conf.public_key,
          conf.secret_key,
          conf.machine_name,
        );
