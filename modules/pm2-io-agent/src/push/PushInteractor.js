...
module.exports = class PushInteractor {
  constructor(opts, ipm2, transport) {
    ...
    this._stackParser = new Utility.StackTraceParser({
      cache: this._cacheFS,
      context: cst.CONTEXT_ON_ERROR,
    });
  ...
  _onPM2Event(event, packet) {
    ...
    if (
      ...
      return false;
    if (
      this.processes.get(packet.process.name) &&
      this.processes.get(packet.process.name)._km_monitored === false
    )
      return false;
    ...
    if (event.match(/^log:/)) {
      ...
      if (
        !this.processes.has(packet.process.name) ||
        this.processes.get(packet.process.name).send_logs === false
      )
        return false;
    }
    ...
    if (
      ...
      packet.data &&
      packet.data.return &&
      (packet.data.return.heapdump ||
        packet.data.return.cpuprofile ||
        packet.data.return.heapprofile)
    ) {
    ...
    packet.process = {
      ...
      rev:
        packet.process.rev ||
        (packet.process.versioning && packet.process.versioning.revision
          ? packet.process.versioning.revision
          : null),
    ...
    this._ipm2.rpc.getMonitorData({}, (err, processes) => {
      ...
      processes.forEach((process) => {
        ...
        this.broadcast_logs.set(
          process.pm_id,
          process.pm2_env.broadcast_logs == 1 || process.pm2_env.broadcast_logs == 'true',
        );
      });
