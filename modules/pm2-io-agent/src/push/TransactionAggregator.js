...
module.exports = class TransactionAggregator {
  ...
  init() {
    ...
    this.pushInteractor._ipm2.bus.on('process:event', (data) => {
      ...
      let rev =
        data.process.versioning && data.process.versioning.revision
          ? data.process.versioning.revision
          : null;
  ...
  mergeTrace(aggregated, trace, process) {
    ...
    const merge = (variance) => {
      ...
      if (variance == null) {
        ...
        if (
          trace.spans[0].mean > variance.histogram.percentiles([0.95])[0.95] &&
          typeof pushInteractor !== 'undefined' &&
          !process.initialization_timeout
        ) {
    ...
    for (let i = 0, len = keys.length; i < len; i++) {
      ...
      for (let j = path.length - 1; j >= 0; j--) {
        ...
        if (path[j] !== segments[j]) {
          ...
          if (
            this.isIdentifier(path[j]) &&
            segments[j] === '*' &&
            path[j - 1] === segments[j - 1]
          ) {
  ...
  prepareAggregationforShipping() {
    ...
    Object.keys(this.processes).forEach((appName) => {
      ...
      this.SPANS_DB.forEach((name) => {
        ...
        if (!histogram) return;
        normalized[appName].data.meta.db_percentiles[name] = fclone(
          histogram.percentiles([0.5])[0.5],
        );
      });
  ...
  isIdentifier(id) {
    ...
    if (
      id.match(
        /[0-9a-f]{8}-[0-9a-f]{4}-[14][0-9a-f]{3}-[0-9a-f]{4}-[0-9a-f]{12}|[0-9a-f]{12}[14][0-9a-f]{19}/i,
      )
    ) {
      return true;
      // if number
