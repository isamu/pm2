...
var randomRoutes = ['/api/bucket', '/api/bucket/users', '/api/bucket/chameau', '/backo/testo'];
...
function generateTrace(routePath, dbQueryNb) {
  if (!dbQueryNb) {
    dbQueryNb = getRandomInt(2, 5);
  }
  if (!routePath) {
    routePath = randomRoutes[getRandomInt(0, randomRoutes.length - 1)];
  }
  ...
  for (var i = 0; i < dbQueryNb; i++) {
    trace.spans[i + 1] = {
      ...
      startTime: dayjs()
        .subtract(dbQueryNb - i + 1, 'seconds')
        .toISOString(),
      endTime: dayjs()
        .subtract(dbQueryNb - i, 'seconds')
        .toISOString(),
    };
  }
