...
describe('PushInteractor', () => {
      ...
      push._ipm2 = {
        bus: {
          on: (event, method) => {
            assert(event === '*');
            if (!isNode4) {
              // method.name doesnt exist in node 4
  ...
  describe('_worker', (_) => {
    ...
    it('should send transport status', (done) => {
      let DataRetrieverMock = new ModuleMocker(
        path.resolve(__dirname, '../../../src/push/DataRetriever'),
      );
