    ...
    child.on('message', (pck) => {
      if (pck.type === 'axm:monitor') {
        ...
        if (hasGCMetrics && hasContextSwitchMetrics && hasPFMetrics) {
          console.log(
            `found GC metrics: ${metricsName.filter((name) => !!name.match(/GC/)).join(',')}`,
          );
          child.kill('SIGINT');
          done();
        }
    ...
    setTimeout((_) => {
      child.on('message', (pck) => {
        if (pck.type === 'axm:monitor') {
          const metricsName = Object.keys(pck.data);
          assert(
            metricsName.every((name) => !name.match(/GC/)),
            'should have no GC metrics',
          );
          assert(
            metricsName.every((name) => !name.match(/Page Fault/)),
            'should have no Page fault metrics',
          );
          assert(
            metricsName.every((name) => !name.match(/Context Switch/)),
            'should have no context switch metrics',
          );
          child.kill('SIGINT');
        }
      });
