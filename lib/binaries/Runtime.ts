...
commander
  .version(pkg.version)
  .description(
    'pm2-runtime is an automatic pmx injection that runs in simulated no-daemon environment',
  )
  .option('--auto-manage', 'keep application online after command exit')
  .option(
    '--fast-boot',
    'boot app faster by keeping pm2 runtime online in background (effective at second exit/start)',
  )
...
commander.command('*').action(function (cmd) {
  ...
  pm2.connect(function () {
    ...
    pm2.start(cmd, commander, function (err, obj) {
      ...
      if (commander.json === true) Log.jsonStream(pm2.Client, pm_id);
...
function exitPM2() {
  ...
  if (commander.autoManage) return process.exit(0);
