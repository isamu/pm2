...
const tabtab = require('../completion.js');
const Common = require('../Common.js');
const PM2ioHandler = require('../API/pm2-plus/PM2IO');
...
commander
  ...
  .option(
    ...
    'specify which set of environment variables from ecosystem file must be injected',
  )
  .option(
    '-a --update-env',
  ...
  .option(
    ...
    'Restart the app if an amount of memory is exceeded (in bytes)',
  )
  .option('--restart-delay <delay>', 'specify a delay between restarts (in milliseconds)')
  .option(
    '--exp-backoff-restart-delay <delay>',
    'specify a delay between restarts (in milliseconds)',
  )
  ...
  .option(
    '--wait-ip',
    'override systemd script to wait for full internet connectivity to launch pm2',
  )
  ...
  .option(
    ...
    '(alias) restart a running process based on a cron pattern',
  )
  ...
  .option(
    ...
    'with json declaration, allow to only act on one application',
  )
  ...
  .option(
    '--watch-delay <delay>',
    'specify a restart delay after changing files (--watch-delay 4 (in sec) or 4000ms)',
  )
  ...
  .option(
    '--stop-exit-codes <exit_codes...>',
    'specify a list of exit codes that should skip automatic restart',
  )
...
function checkCompletion() {
  return tabtab.complete('pm2', function (err, data) {
    ...
    if (/^--\w?/.test(data.last))
      return tabtab.log(
        commander.options.map(function (data) {
          return data.long;
        }),
        data,
      );
    ...
    if (cmdProcess.indexOf(data.prev) > -1) {
      ...
      tabtab.log(
        commander.commands.map(function (data) {
          return data._name;
        }),
        data,
      );
...
//
commander
  .command('start [name|namespace|file|ecosystem|id...]')
  ...
  .option(
    '--dist',
    'with --container; change local Dockerfile to containerize all files in current directory',
  )
  ...
  .action(function (cmd, opts) {
    ...
    else if (opts.container == true) return pm2.dockerMode(cmd, opts, 'development');
...
//
commander
  .command('stop <id|name|namespace|all|json|stdin...>')
...
//
commander
  .command('restart <id|name|namespace|all|json|stdin...>')
...
//
commander
  .command('reload <id|name|namespace|all>')
...
//
commander
  .command('delete <name|id|namespace|script|all|json|stdin...>')
  .alias('del')
...
//
commander
  .command('sendSignal <signal> <pm2_id|name>')
...
commander
  ...
  .action(function (id, cmd) {
    ...
    if (cmd.nostream === true) pm2.printLogs(id, line, raw, timestamp, exclusive);
    else if (cmd.json === true) Logs.jsonStream(pm2.Client, id);
...
commander
  ...
  .option(
    ...
    'frontend app monitoring (auto integrate snippet on html files)',
  )
...
commander.command('autoinstall').action(function () {
  pm2.autoinstall();
});
...
commander
  ...
  .action(function () {
    ...
    try {
      ...
      Common.printError(
        cst.PREFIX_MSG_ERR + 'Failed to remove OpenTelemetry packages: ' + e.message,
      );
      process.exit(cst.ERROR_EXIT);
    }
    process.exit(cst.SUCCESS_EXIT);
