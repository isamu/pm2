...
Daemon.prototype.start = function () {
  ...
  d.once('error', function (err) {
    ...
    const fork_new_pm2 = require('child_process').spawn('node', [path, 'update'], {
      detached: true,
      windowsHide: true,
      stdio: 'inherit',
    });
    ...
    fs.chmod(that.pub_socket_file, '775', function (e) {
      ...
      try {
        if (process.env.PM2_SOCKET_USER && process.env.PM2_SOCKET_GROUP)
          fs.chown(
            that.pub_socket_file,
            parseInt(process.env.PM2_SOCKET_USER),
            parseInt(process.env.PM2_SOCKET_GROUP),
            function (e) {
              if (e) console.error(e);
            },
          );
...
Daemon.prototype.startLogic = function () {
  ...
  God.bus.on('axm:option:configuration', function axmMonitor(msg) {
    ...
    try {
      ...
      Object.keys(msg.data).forEach(function (conf_key) {
        God.clusters_db[msg.process.pm_id].pm2_env.axm_options[conf_key] = Utility.clone(
          msg.data[conf_key],
        );
      });
    } catch (e) {
  ...
  God.bus.onAny(function (event, data_v) {
    if (
      ['axm:action', 'axm:monitor', 'axm:option:setPID', 'axm:option:configuration'].indexOf(
        event,
      ) > -1
    ) {
      data_v = null;
...
if (require.main === module) {
  process.title =
    process.env.PM2_DAEMON_TITLE ||
    'PM2 v' + pkg.version + ': God Daemon (' + process.env.PM2_HOME + ')';
