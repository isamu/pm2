...
Common.prepareAppConf = function (opts, app) {
   ...
   */
  if (cst.PM2_PROGRAMMATIC || process.env.pm_id) Common.safeExtend(env, process.env);
  ...
  ['log', 'out', 'error', 'pid'].forEach(function (f) {
    ...
    if (af !== 'NULL' && af !== '/dev/null') {
      ps &&
        (app['pm_' + (isStd ? f.substr(0, 3) + '_' : '') + ext + '_path'] = path.resolve.apply(
          null,
          ps,
        ));
    } else if (path.sep === '\\') {
...
Common.sink.determineExecMode = function (app) {
   ...
   */
  const interpreterBasename = path.basename(app.exec_interpreter || '');
  if (
    !app.exec_mode &&
    (app.instances >= 1 || app.instances === 0 || app.instances === -1) &&
    (interpreterBasename === 'node' ||
      /node$/.test(interpreterBasename) ||
      interpreterBasename === 'bun' ||
      /bun$/.test(interpreterBasename))
  ) {
    ...
    if (betterInterpreter == 'python') {
      if (which('python') == null) {
        if (which('python3') == null)
          Common.printError(
            cst.PREFIX_MSG_WARNING +
              chalk.bold.yellow('python and python3 binaries not available in PATH'),
          );
...
function cropToWidth(str, width) {
  ...
  while (i < str.length) {
    ...
    if (visible >= width - 1) return out + '\u2026\x1b[0m';
    out += str[i];
    visible++;
    i++;
  }
...
Common.mergeEnvironmentVariables = function (app_env, env_name, deploy_conf) {
  ...
  if (env_name) {
    ...
    if ('env_' + env_name in app) {
      ...
      Common.printOut(
        cst.PREFIX_MSG_WARNING + chalk.bold('Environment [%s] is not defined in process file'),
        env_name,
      );
    }
  }
  ...
  for (let i = 0; i < appConfs.length; i++) {
    ...
    if (app.trace === true || app.deep_monitoring === true) {
      ...
      if (!OtelManager.ensureInstalled()) {
        Common.printError(
          cst.PREFIX_MSG_ERR + 'Tracing disabled: OpenTelemetry packages not available',
        );
        app.trace = false;
