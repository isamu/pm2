...
function create() {
  ...
  function cpuUsage() {
    ...
    let pct = 0;
    if (prev.cpu) {
      const dIdle = idle - prev.cpu.idle;
      const dTotal = total - prev.cpu.total;
  ...
  async function ram(): Promise<MemorySample> {
    ...
    if (IS_LINUX) {
      try {
        ...
        /* fall back to os values */
      }
    } else if (IS_DARWIN) {
      const ps = await execFileP('sysctl', ['-n', 'vm.pagesize']);
      const pageSize = parseInt((ps || '').trim(), 10) || 4096;
      const vm = await execFileP('vm_stat', []);
      if (vm) {
        const ma = vm.match(/Pages active:\s+(\d+)/);
        const mi2 = vm.match(/Pages inactive:\s+(\d+)/);
        if (ma) active = parseInt(ma[1], 10) * pageSize;
        const inactive = mi2 ? parseInt(mi2[1], 10) * pageSize : 0;
        available = total - active + inactive;
      }
    }

    const usagePct = total > 0 ? (active / total) * 100 : 0;
  ...
  function readLinuxNet(name: string): Counters | null {
    ...
    try {
      ...
      return {
        ...
        rxErr: rd('rx_errors'),
        txErr: rd('tx_errors'),
        rxDrop: rd('rx_dropped'),
        txDrop: rd('tx_dropped'),
      };
    } catch (e) {
      return null;
    }
  ...
  function readLinuxDisk() {
    try {
      ...
      devs.forEach(function (d: string) {
        ...
        try {
          const st = fs
            .readFileSync('/sys/block/' + d + '/stat', 'utf8')
            .trim()
            .replace(/ +/g, ' ')
            .split(' ');
  ...
  async function readDarwinDisk() {
    const out = await execFileP('ioreg', [
      '-c',
  ...
  async function fsUsage(): Promise<FsSample[]> {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const sizeKb = parseInt(p[1], 10) || 0;
      const usedKb = parseInt(p[2], 10) || 0;
      const availKb = parseInt(p[3], 10) || 0;
      const mount = p.slice(5).join(' ');
      const denom = usedKb + availKb;
      if (denom <= 0) continue;
      res.push({
        mount: mount,
  ...
  function collect(cb?: (metrics: Record<string, unknown>) => void) {
    return Promise.all([
      ...
      diskIO().catch(function () {
        return null;
      }),
      fsUsage().catch(function () {
        return [];
      }),
    ])
      .then(function (r) {
        const m = buildSnapshot(r[0], r[1], r[2], r[3], r[4], Date.now());
        if (cb) cb(m);
        return m;
      })
      .catch(function () {
