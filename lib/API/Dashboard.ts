...
const blessed = require('@pm2/blessed');
const debug = require('debug')('pm2:monit');
...
Dashboard.init = function () {
  ...
  this.screen.key(['left', 'right'], function (ch, key) {
    ...
    if (key.name === 'left') {
      if (i == 3) that[boards[0]].style.border.fg = 'white';
      else that[boards[i + 1]].style.border.fg = 'white';
    } else {
      if (i == 0) that[boards[3]].style.border.fg = 'white';
      else that[boards[i - 1]].style.border.fg = 'white';
    }
  });
...
Dashboard.refresh = function (processes) {
  ...
  for (let i = 0; i < processes.length; i++) {
    ...
    let name = processes[i].pm2_env.name || p.basename(processes[i].pm2_env.pm_exec_path);
    const maxNameLen = 15;
    if (name.length > maxNameLen) name = name.substring(0, maxNameLen - 1) + '…';
    name = name.padEnd(maxNameLen);

    // Line of list
    const memMB = (processes[i].monit.memory / 1048576).toFixed(0);
    const cpu = processes[i].monit.cpu;
    const memColor = gradient(memPercent, [255, 0, 0], [0, 255, 0]);
    const cpuColor = gradient(cpu, [255, 0, 0], [0, 255, 0]);
    const item = `[${String(processes[i].pm2_env.pm_id).padStart(2)}] ${name} Mem: {bold}{${memColor}-fg}${String(memMB).padStart(3)}{/} MB  CPU: {bold}{${cpuColor}-fg}${String(cpu).padStart(2)}{/} %  ${status}`;
    ...
    this.metadataBox.setLine(
      1,
      'Namespace             ' + '{bold}' + proc.pm2_env.namespace + '{/}',
    );
    ...
    this.metadataBox.setLine(
      4,
      'Uptime                ' +
        (proc.pm2_env.pm_uptime && proc.pm2_env.status == 'online'
    ...
    this.metadataBox.setLine(
      ...
      'Script args           ' +
        (proc.pm2_env.args
          ? (typeof proc.pm2_env.args == 'string'
              ? JSON.parse(proc.pm2_env.args.replace(/'/g, '"'))
              : proc.pm2_env.args
            ).join(' ')
          : 'N/A'),
    );
    ...
    this.metadataBox.setLine(
      8,
      'Interpreter args      ' +
        (proc.pm2_env.node_args.length != 0 ? proc.pm2_env.node_args : 'N/A'),
    );
    this.metadataBox.setLine(
      9,
      'Exec mode             ' +
        (proc.pm2_env.exec_mode == 'fork_mode' ? '{bold}fork{/}' : '{blue-fg}{bold}cluster{/}'),
    );
    this.metadataBox.setLine(10, 'Node.js version       ' + proc.pm2_env.node_version);
    this.metadataBox.setLine(
      11,
      'watch & reload        ' +
        (proc.pm2_env.watch ? '{green-fg}{bold}✔{/}' : '{red-fg}{bold}✘{/}'),
    );
    ...
    this.metadataBox.setLine(
      13,
      'Comment               ' +
        (proc.pm2_env.versioning ? proc.pm2_env.versioning.comment : 'N/A'),
    );
    this.metadataBox.setLine(
      14,
      'Revision              ' +
        (proc.pm2_env.versioning ? proc.pm2_env.versioning.revision : 'N/A'),
    );
    this.metadataBox.setLine(
      15,
    ...
    this.metadataBox.setLine(
      16,
      'Remote url            ' + (proc.pm2_env.versioning ? proc.pm2_env.versioning.url : 'N/A'),
    );
    this.metadataBox.deleteLine(17);
    this.metadataBox.setLine(
      17,
      'Last update           ' +
        (proc.pm2_env.versioning ? proc.pm2_env.versioning.update_time : 'N/A'),
    );
    ...
    for (const key in proc.pm2_env.axm_monitor) {
      const metric_name = proc.pm2_env.axm_monitor[key].hasOwnProperty('value')
        ? proc.pm2_env.axm_monitor[key].value
        : proc.pm2_env.axm_monitor[key];
      const metric_unit = proc.pm2_env.axm_monitor[key].hasOwnProperty('unit')
        ? proc.pm2_env.axm_monitor[key].unit
        : null;
