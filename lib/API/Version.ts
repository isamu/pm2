...
module.exports = function (CLI) {
  ...
  CLI.prototype._pull = function (opts, cb) {
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      if (err || processes.length === 0) {
        ...
        return cb
          ? cb({ msg: 'Process not found: ' + process_name })
      ...
      require('../../modules/vizion').update(
        ...
        function (err, meta) {
          ...
          if (meta.success === true) {
            ...
            printOut(
              cst.PREFIX_MSG + 'Already up-to-date or an error occured for app: %s',
              process_name,
            );
            return cb
              ? cb({ success: false, msg: 'Already up to date' })
              : that.exitCli(cst.SUCCESS_EXIT);
          }
          return false;
        },
  ...
  CLI.prototype.pullCommitId = function (process_name, commit_id, cb) {
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      ...
      const proc = processes[0];
      if (proc.pm2_env.versioning) {
        require('../../modules/vizion').isUpToDate(
          { folder: proc.pm2_env.versioning.repo_path },
          function (err, meta) {
            if (err !== null) return cb ? cb({ msg: err }) : that.exitCli(cst.ERROR_EXIT);
            require('../../modules/vizion').revertTo(
              { revision: commit_id, folder: proc.pm2_env.versioning.repo_path },
              function (err2, meta2) {
                if (!err2 && meta2.success) {
                      ...
                      execCommands(
                        ...
                        function (err, res) {
                          if (err !== null) {
                            ...
                            printOut(
                              cst.PREFIX_MSG + 'Process successfully updated %s',
                              process_name,
                            );
                  ...
                  printOut(
                    cst.PREFIX_MSG + 'Already up-to-date or an error occured: %s',
                    process_name,
                  );
  ...
  CLI.prototype.backward = function (process_name, cb) {
    const that = this;
    printOut(
      cst.PREFIX_MSG + 'Downgrading to previous commit repository for process name %s',
      process_name,
    );
  ...
  CLI.prototype.forward = function (process_name, cb) {
    const that = this;
    printOut(
      cst.PREFIX_MSG + 'Updating to next commit repository for process name %s',
      process_name,
    );
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      ...
      if (proc.pm2_env.versioning) {
        ...
        printOut(cst.PREFIX_MSG + 'No versioning system found for process %s', process_name);
        return cb
          ? cb({ success: false, msg: 'No versioning system found' })
          : that.exitCli(cst.SUCCESS_EXIT);
      }
    });
  };
  ...
  var getPostUpdateCmds = function (repo_path, proc_name, cb) {
    ...
    eachSeries(
      ['ecosystem.json', 'process.json', 'package.json'],
      searchForCommands,
      function (final) {
