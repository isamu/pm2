...
function miniMonitBar(sys_infos) {
  ...
  interfaces.forEach((iface) => {
    ...
    let rx_err = sys_metrics[`net:rx_errors_60:${iface}`]
      ? Number(sys_metrics[`net:rx_errors_60:${iface}`].value)
      : 0;
    let tx_err = sys_metrics[`net:tx_errors_60:${iface}`]
      ? Number(sys_metrics[`net:tx_errors_60:${iface}`].value)
      : 0;
    let rx_drop = sys_metrics[`net:rx_dropped_60:${iface}`]
      ? Number(sys_metrics[`net:rx_dropped_60:${iface}`].value)
      : 0;
    let tx_drop = sys_metrics[`net:tx_dropped_60:${iface}`]
