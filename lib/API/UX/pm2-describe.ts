...
module.exports = function (proc) {
  ...
  UxHelpers.safe_push(
    ...
    {
      uptime:
        pm2_env.pm_uptime && pm2_env.status == 'online'
          ? UxHelpers.timeSince(pm2_env.pm_uptime)
          : 0,
    },
    { 'script path': pm2_env.pm_exec_path },
    {
      'script args': pm2_env.args
  ...
  if (pm2_env.axm_actions && Object.keys(pm2_env.axm_actions).length > 0) {
    ...
    Common.printOut(
      chalk.white.italic(' Trigger via: pm2 trigger %s <action_name>\n'),
      pm2_env.name,
    );
  }
  ...
  if (pm2_env.axm_monitor && Object.keys(pm2_env.axm_monitor).length > 0) {
    ...
    Object.keys(pm2_env.axm_monitor).forEach(function (key) {
      const obj = {};
      const metric_name = pm2_env.axm_monitor[key].hasOwnProperty('value')
        ? pm2_env.axm_monitor[key].value
        : pm2_env.axm_monitor[key];
      const metric_unit = pm2_env.axm_monitor[key].hasOwnProperty('unit')
        ? pm2_env.axm_monitor[key].unit
        : '';
  ...
  Common.printOut(
    chalk.white.italic(' Use `pm2 logs %s [--lines 1000]` to display logs'),
    pm2_env.name,
  );
  Common.printOut(
    chalk.white.italic(' Use `pm2 env %s` to display environment variables'),
    pm2_env.pm_id,
  );
  Common.printOut(
    chalk.white.italic(' Use `pm2 monit` to monitor CPU and Memory usage'),
    pm2_env.name,
  );
};
