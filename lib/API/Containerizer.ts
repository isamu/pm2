...
const Promise = require('../tools/promise.min.js');
    ...
    exec("docker version -f '{{.Client.Version}}'", function (err, stdout, stderr) {
      if (err) {
        ...
        if (err.message && err.message.indexOf('Cannot connect to the Docker') > -1) {
          console.log();
          console.log(
            chalk.blue.bold('[Solution] Setup Docker to be able to be used without sudo rights:'),
          );
...
function parseAndSwitch(file_content, main_file, opts) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ['## DISTRIBUTION MODE', '## DEVELOPMENT MODE'].indexOf(line) > -1 ||
      i == lines.length - 1
    ) {
...
function handleExit(CLI, opts, mode) {
  process.on('SIGINT', function () {
    ...
    exec('docker ps -lq', function (err, stdout, stderr) {
      ...
      require('../../modules/vizion').analyze(
        { folder: process.cwd() },
        function recur_path(err, meta) {
          if (!err && meta.revision) {
            const commit_id = util.format(
              '#%s(%s) %s',
              meta.branch,
            ...
            console.log(
              chalk.bold.magenta('$ docker commit %s %s'),
              stdout.replace('\n', ''),
              opts.imageName,
            );
...
module.exports = function (CLI) {
  CLI.prototype.generateDockerfile = function (script, opts) {
    ...
    fs.stat(docker_filepath, function (err, stat) {
      ...
      console.log(
        chalk.red.bold(
          'Dockerfile already exists in this folder, use --force if you want to replace it',
        ),
      );
        ...
        return new Promise(function (resolve, reject) {
          ...
          fs.stat(docker_filepath, function (err, stat) {
            if (err) {
              ...
              promptly.prompt(
                'No Dockerfile in current directory, ok to generate a new one? (y/n)',
                function (err, value) {
                  if (value == 'y')
                    return resolve(
                      generateDockerfile(docker_filepath, main_script, {
