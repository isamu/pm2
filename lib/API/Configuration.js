...
module.exports = function (CLI) {
  ...
  CLI.prototype.multiset = function (serial, cb) {
    ...
    Configuration.multiset(serial, function (err, data) {
      if (err) return cb ? cb({ success: false, err: err }) : that.exitCli(cst.ERROR_EXIT);
      ...
      // No app to scope to: the key had a single segment, so app_name above was never reached.
