...
const debug = require('debug')('pm2:serve');
...
function serveFile(uri, request, response) {
  let file = decodeURIComponent(new URL(uri || request.url, 'http://localhost').pathname);
...
function renderDirectoryListing(dirPath, urlPath, items, response) {
  ...
  if (urlPath !== '/') {
    rows +=
      '<tr><td class="name"><a href="../">../</a></td><td class="size">-</td><td class="mtime">-</td></tr>\n';
  }

  items.forEach(function (item) {
    const displayName = item.isDir ? item.name + '/' : item.name;
    const href = encodeURIComponent(item.name) + (item.isDir ? '/' : '');
    const size = item.isDir ? '-' : formatFileSize(item.size);
    const mtime = item.mtime.toISOString().replace('T', ' ').slice(0, 19);
    rows +=
      '<tr>' +
