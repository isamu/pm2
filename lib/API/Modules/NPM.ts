...
function localStart(PM2, opts, cb) {
  ...
  Common.printOut(
    cst.PREFIX_MSG_MOD + 'Installing local module in DEVELOPMENT MODE with WATCH auto restart',
  );
...
function generateSample(app_name, cb) {
  ...
  function samplize(module_name) {
    const cmd1 =
      'git clone https://github.com/pm2-hive/sample-module.git ' +
      module_name +
      '; cd ' +
      module_name +
      '; rm -rf .git';
    const cmd2 =
      'cd ' + module_name + ' ; sed -i "s:sample-module:' + module_name + ':g" package.json';
    ...
    sexec(cmd1, function (err) {
      ...
      sexec(cmd2, function (err) {
        ...
        sexec(cmd3, function (err) {
          console.log('');
          Common.printOut(
            cst.PREFIX_MSG_MOD + 'Module sample created in folder: ',
            path.join(process.cwd(), module_name),
          );
          console.log('');
...
function publish(opts, cb) {
  ...
  Common.printOut(
    cst.PREFIX_MSG_MOD + 'Incrementing module to: %s@%s',
    package_json.name,
  ...
  rl.question('Write & Publish? [Y/N]', function (answer) {
    ...
    fs.writeFile(package_file, JSON.stringify(package_json, null, 2), function (err, data) {
      ...
      Common.printOut(
        cst.PREFIX_MSG_MOD + 'Publishing module - %s@%s',
        package_json.name,
        package_json.version,
      );
      ...
      sexec('npm publish', function (code) {
        Common.printOut(
          cst.PREFIX_MSG_MOD + 'Module - %s@%s successfully published',
          package_json.name,
          package_json.version,
        );
        ...
        sexec(
          'git add . ; git commit -m "' + package_json.version + '"; git push origin master',
          function (code) {
            Common.printOut(
              cst.PREFIX_MSG_MOD + 'Installable with pm2 install %s',
              package_json.name,
            );
...
function getNPMCommandLine(module_name, install_path) {
  if (which('bun')) {
    return spawn.bind(
      this,
      'bun',
      ['install', module_name, '--loglevel=error', '--cwd', install_path],
      {
        stdio: 'inherit',
        env: process.env,
        windowsHide: true,
    ...
    return spawn.bind(
      ...
      ['install', module_name, '--loglevel=error', '--prefix', install_path],
      {
        stdio: 'inherit',
        env: process.env,
        windowsHide: true,
    ...
    return spawn.bind(
      this,
      cst.BUILTIN_NODE_PATH,
      [cst.BUILTIN_NPM_PATH, 'install', module_name, '--loglevel=error', '--prefix', install_path],
      {
        stdio: 'inherit',
        env: process.env,
        windowsHide: true,
...
function continueInstall(CLI, module_name, opts, cb) {
  ...
  fs.promises.mkdir(install_path, { recursive: true }).then(function () {
  ...
  function finalizeInstall(code) {
    if (code != 0) {
      ...
      return Rollback.revert(CLI, module_name, function () {
        return cb(
          new Error('Installation failed via NPM, module has been restored to prev version'),
        );
      });
    }
    ...
    Configuration.set(
      cst.MODULE_CONF_PREFIX + ':' + canonic_module_name,
      {
        uid: opts.uid,
        gid: opts.gid,
      },
...
function StartModule(CLI, opts, cb) {
  ...
  CLI.start(package_json, opts, function (err, data) {
    ...
    if (opts.safe) {
      Common.printOut(
        cst.PREFIX_MSG_MOD + 'Monitoring module behavior for potential issue (5secs...)',
      );
...
var Rollback = {
  revert: function (CLI, module_name, cb) {
    ...
    Common.printOut(
      cst.PREFIX_MSG_MOD + chalk.bold.red('[[[[[ Module installation failure! ]]]]]'),
    );
    ...
    CLI.deleteModule(canonic_module_name, function () {
      ...
      if (module_name.includes('modules') === true) deleteFolderRecursive(module_path);
