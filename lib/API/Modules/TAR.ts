...
function installLocal(PM2, module_filepath, opts, cb) {
  ...
  getModuleName(module_filepath, function (err, module_name) {
    ...
    const install_instance = spawn(
      'tar',
      ['zxf', module_filepath, '-C', install_path, '--strip-components', '1'],
      {
        stdio: 'inherit',
        env: process.env,
      },
...
function uninstall(PM2, module_name, cb) {
  ...
  forEachLimit(
    ...
    (app, next) => {
      ...
      if (apps.length > 1) app_name = `${module_name}:${app.name}`;
...
function getModuleName(module_filepath, cb) {
  ...
  const install_instance = spawn(
    'tar',
    ['zxf', module_filepath, '-C', os.tmpdir(), cst.MODULE_BASEFOLDER + '/package.json'],
    {
      stdio: 'inherit',
      env: process.env,
    },
  ...
  install_instance.on('close', function (code) {
    try {
      const pkg = JSON.parse(fs.readFileSync(path.join(tmp_folder, `package.json`)));
...
function publish(PM2, folder, cb) {
  ...
  packager(current_path, target_path, (err, res) => {
    ...
    const fileBuffer = fs.readFileSync(res.path);
    formData.append(
      'module_data',
      new Blob([fileBuffer], { type: 'content/gzip' }),
