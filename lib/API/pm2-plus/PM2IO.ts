...
const CLIStrategy = require('./auth-strategies/CliAuth');
const WebStrategy = require('./auth-strategies/WebAuth');
...
module.exports = class PM2ioHandler {
  ...
  static strategy() {
    switch (process.platform) {
      ...
      case 'linux': {
        const isDesktop =
          process.env.XDG_CURRENT_DESKTOP || process.env.XDG_SESSION_DESKTOP || process.env.DISPLAY;
  ...
  static launch(command, opts) {
    ...
    switch (command) {
      ...
      case 'web': {
        this.#strategy
          ...
          .then((res) => {
            ...
            if (res === false) {
              console.error(
                `${cst.PM2_IO_MSG_ERR} You need to be authenticated to do that, please use: pm2 plus login`,
              );
              return process.exit(1);
            }
      ...
      default: {
        console.log(
          `${cst.PM2_IO_MSG_ERR} Invalid command ${command}, available : login,register,validate,connect or web`,
        );
        process.exit(1);
      }
    }
  ...
  static openUI() {
    this.#io.bucket.retrieveAll().then((res) => {
      ...
      if (buckets.length === 0) {
        return this.createBucket((err, bucket) => {
          if (err) {
            ...
            if (bucket) {
              console.error(
                `${cst.PM2_IO_MSG_ERR} You can retry using: pm2 plus link ${bucket.secret_id} ${bucket.public_id}`,
              );
            }
            console.error(
              `${cst.PM2_IO_MSG_ERR} You can also contact us to get help: contact@pm2.io`,
      ...
      promptly.choose(
        `${cst.PM2_IO_MSG} Type the name of the bucket you want to connect to :`,
        choices,
        (err, value) => {
