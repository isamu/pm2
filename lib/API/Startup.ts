...
const sexec = require('../tools/sexec');
module.exports = function (CLI) {
  ...
  function isNotRoot(startup_mode, platform, opts, cb) {
    Common.printOut(
      `${cst.PREFIX_MSG}To ${startup_mode} the Startup Script, copy/paste the following command:`,
    );
      ...
      function fin(err) {
        ...
        if (!force && env_arr.length === 0 && !process.env.FORCE) {
          ...
          if (cb) {
            ...
            Common.printOut(
              cst.PREFIX_MSG_WARNING + 'PM2 is not managing any process, skipping save...',
            );
        ...
        try {
          ...
          Common.printOut(
            cst.PREFIX_MSG_ERR + 'Failed to back up dump file in %s',
            cst.DUMP_BACKUP_FILE_PATH,
          );
        }

        // Overwrite dump file, delete if broken and exit
        try {
          fs.writeFileSync(cst.DUMP_FILE_PATH, JSON.stringify(env_arr, null, 2));
        } catch (e) {
          console.error(e.stack || e);
          try {
            // try to backup file
          ...
          Common.printOut(
            cst.PREFIX_MSG_ERR + 'Failed to save dump file in %s',
            cst.DUMP_FILE_PATH,
          );
