...
exports.parseOut = function parseOut(str) {
  // match answers null rather than an empty list, so help output carrying no flags of one kind
  // used to take the completion script down here.
...
function install(name, completer, cb) {
  ...
  readRc(completer, function (err, file) {
    ...
    if (part) {
      return cb(
        null,
        ' ✗ ' + completer + ' tab-completion has been already installed. Do nothing.',
      );
    }
...
function uninstall(name, completer, cb) {
  ...
  readRc(completer, function (err, file) {
    ...
    if (!part) {
      return cb(
        null,
        ' ✗ ' + completer + ' tab-completion has been already uninstalled. Do nothing.',
      );
    }
...
function rcFileName() {
  return '.' + process.env.SHELL.match(/\/bin\/(\w+)/)[1] + 'rc';
}

exports.missingRcError = function missingRcError(file, completer) {
  return new Error(
    'No ' + file + " file. You'll have to run instead: " + completer + ' completion >> ~/' + file,
  );
};

function withRc(completer, action) {
  var file = rcFileName(),
    filepath = pth.join(process.env.HOME, file);
  fs.lstat(filepath, function (err) {
    if (err) return action(exports.missingRcError(file, completer));
    action(null, filepath);
  });
}

function readRc(completer, cb) {
  withRc(completer, function (err, filepath) {
    if (err) return cb(err);
