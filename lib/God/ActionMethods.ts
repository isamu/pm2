...
module.exports = function (God) {
  ...
  God.startProcessId = function (id, cb) {
    ...
    if (proc.process && proc.process.pid)
      return cb(
        God.logAndGenerateError('Process with pid ' + proc.process.pid + ' already exists'),
        {},
      );
  ...
  God.restartProcessId = function (opts, cb) {
    ...
    if (God.pm2_being_killed) {
      return cb(
        God.logAndGenerateError(
          '[RestartProcessId] PM2 is being killed, stopping restart procedure...',
        ),
      );
    }
    ...
    if ('id' in cmd) {
      ...
      (function ex(arr) {
        ...
        const isActionAvailable =
          proc_env.axm_actions.find((action) => action.action_name === cmd.msg) !== undefined;
        ...
        if (
          (p.basename(proc_env.pm_exec_path) == name ||
            proc_env.name == name ||
