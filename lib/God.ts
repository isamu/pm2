...
const fs = require('fs');
const vizion = require('../modules/vizion');
...
God.executeApp = function executeApp(env, cb) {
  ...
  if (env_copy['autostart'] === false) {
    const clu = { pm2_env: env_copy, process: { pid: 0 } };
...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  ...
  let stopExitCodes =
    proc.pm2_env.stop_exit_codes !== undefined && proc.pm2_env.stop_exit_codes !== null
      ? proc.pm2_env.stop_exit_codes
      : [];
  ...
  const min_uptime =
    typeof proc.pm2_env.min_uptime !== 'undefined' ? proc.pm2_env.min_uptime : 1000;
  const max_restarts =
    typeof proc.pm2_env.max_restarts !== 'undefined' ? proc.pm2_env.max_restarts : 16;
...
God.injectVariables = function injectVariables(env, cb) {
  ...
  if (env.increment_var) {
    const lastIncrement = Object.keys(God.clusters_db)
      ...
      .filter(function (proc) {
        return (
          proc.pm2_env.name === env.name && typeof proc.pm2_env[env.increment_var] !== 'undefined'
        );
      })
      .map(function (proc) {
