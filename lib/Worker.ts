...
const SysMetrics = require('./tools/SysMetrics.js');
...
module.exports = function (God) {
  ...
  God.registerCron = function (pm2_env) {
    ...
    const pm_id = pm2_env.pm_id;
    console.log('[PM2][WORKER] Registering a cron job on:', pm_id);

    const job = Cron(pm2_env.cron_restart, function () {
  ...
  const maxMemoryRestart = function (proc_key, cb) {
    ...
    if (
      ...
      console.log(
        '[PM2][WORKER] Process %s restarted because it exceeds --max-memory-restart value (current_memory=%s max_memory_limit=%s [octets])',
        proc.pm2_env.pm_id,
      ...
      eachLimit(
        ...
        function (proc, next) {
          ...
          if (
            ...
            proc.pm2_env.prev_restart_delay &&
            ...
            if (app_uptime > cst.EXP_BACKOFF_RESET_TIMER) {
              ...
              console.log(
                `[PM2][WORKER] Reset the restart delay, as app ${proc.name} has been up for more than ${cst.EXP_BACKOFF_RESET_TIMER}ms`,
              );
            }
          }
  ...
  God.Worker.stop = function () {
    ...
    if (sysMetricsTimer !== null) clearInterval(sysMetricsTimer);
  };
};
