...
const rpc = require('../modules/pm2-axon-rpc');
const forEach = require('async/forEach');
const axon = require('../modules/pm2-axon');
...
const Client = (module.exports = function (opts) {
  if (!opts) opts = {};

  if (!opts.conf) this.conf = require('../constants.js');
  else {
    this.conf = opts.conf;
  }
...
Client.prototype.close = function (cb) {
  ...
  forEach(
    [that.disconnectRPC.bind(that), that.disconnectBus.bind(that)],
    function (fn, next) {
      fn(next);
    },
    cb,
...
Client.prototype.launchDaemon = function (opts, cb) {
  ...
  if (this.conf.IS_BUN === false) child.unref();
  ...
  client.sock.once('error', function (e) {
    if (e.code === 'EACCES') {
      fs.stat(that.conf.DAEMON_RPC_PORT, function (e, stats) {
        if (stats.uid === 0) {
          console.error(
            that.conf.PREFIX_MSG_ERR + 'Permission denied, to give access to current user:',
          );
          console.log(
            '$ sudo chown ' +
              process.env.USER +
              ...
              process.env.USER +
              ' ' +
              that.conf.DAEMON_RPC_PORT +
              ' ' +
              that.conf.DAEMON_PUB_PORT,
          );
        } else
          console.error(
            that.conf.PREFIX_MSG_ERR +
              'Permission denied, check permissions on ' +
              that.conf.DAEMON_RPC_PORT,
          );
...
Client.prototype.getAllProcessIdWithoutModules = function (cb) {
  ...
  this.executeRemote('getMonitorData', {}, function (err, procs) {
    ...
    const proc_ids = procs.filter((proc) => !proc.pm2_env.pmx_module).map((proc) => proc.pm_id);
