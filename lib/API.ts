...
let conf = require('../constants.js');
const Client = require('./Client');
const Common = require('./Common');
const KMDaemon = require('../modules/pm2-io-agent/src/InteractorClient');
...
class API {
  ...
  update(cb) {
    ...
    that.getVersion(function (err, new_version) {
      ...
      if (!that.gl_is_km_linked && !err && pkg.version != new_version) {
        const dt = fs.readFileSync(path.join(__dirname, that._conf.PM2_UPDATE));
          ...
          that.Client.executeRemote(action_name, opts, function (err, res) {
            ...
            res.forEach(function (proc) {
              ...
              if (action_name == 'stopProcessId' && proc.pm2_env && proc.pm2_env.cron_restart) {
                Common.warn(
                  `App ${chalk.bold(proc.pm2_env.name)} stopped but CRON RESTART is still UP ${proc.pm2_env.cron_restart}`,
                );
              }
  ...
  _handleAttributeUpdate(opts) {
    const conf = Config.filterOptions(opts);
    const that = this;
    ...
    function doList(err, list) {
      ...
      if (process.stdout.isTTY === false) {
        ...
        if (that.gl_interact_infos) {
          let dashboard_url = `https://app.pm2.io/#/r/${that.gl_interact_infos.public_key}`;
      ...
      // if (process.stdout.isTTY) if looking for start logs
        ...
        Common.info(
          `Log streaming apps id: ${chalk.cyan(acted.join(' '))}, exit with Ctrl-C or will exit in 10secs`,
        );
  ...
  deepUpdate(cb) {
    ...
    const child = sexec('npm i -g pm2@latest; pm2 update');
