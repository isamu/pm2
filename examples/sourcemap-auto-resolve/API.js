...
API.prototype.start = function (cmd, opts, cb) {
  ...
  if (util.isArray(opts.watch) && opts.watch.length === 0)
    opts.watch =
      (opts.rawArgs ? !!~opts.rawArgs.indexOf('--watch') : !!~process.argv.indexOf('--watch')) ||
      false;
...
API.prototype.delete = function (process_name, jsonVia, cb) {
  ...
  else that._operate('deleteProcessId', process_name, cb);
};
...
API.prototype._startScript = function (script, opts, cb) {
  ...
  if (!!opts.executeCommand) app_conf.exec_mode = 'fork';
  else if (opts.instances !== undefined) app_conf.exec_mode = 'cluster';
  ...
  function restartExistingProcessPath(cb) {
    that.Client.executeRemote('getMonitorData', {}, function (err, procs) {
      ...
      if (
        ...
        Common.printError(
          conf.PREFIX_MSG_ERR + 'Script already launched, add -f option to force re-execution',
        );
  ...
  series(
    [restartExistingProcessName, restartExistingProcessId, restartExistingProcessPath],
    function (err, data) {
    ...
    eachLimit(
      ...
      function (err) {
        ...
        if (apps_name.length > 0 && action != 'start')
          Common.printOut(
            conf.PREFIX_MSG_WARNING + 'Applications %s not running, starting...',
            apps_name.join(', '),
          );
        // Start missing apps
    ...
    eachLimit(
      ...
      function (id, next) {
        ...
        if (
          ...
          if (update_env === true) {
            if (conf.PM2_PROGRAMMATIC == true) new_env = Common.safeExtend({}, process.env);
  ...
  that.Client.executeRemote('getMonitorData', {}, function (err, list) {
    ...
    if (process.stdout.isTTY === false) {
      ...
      if (that.gl_interact_infos) {
        Common.printOut(
          chalk.green.bold('●') +
            ' Agent Online | Dashboard Access: ' +
            chalk.bold('https://app.keymetrics.io/#/r/%s') +
            ' | Server name: %s',
          that.gl_interact_infos.public_key,
          that.gl_interact_infos.machine_name,
        );
      }
      UX.dispAsTable(list, commander);
      Common.printOut(
        chalk.white.italic(' Use `pm2 show <id|name>` to get more details about an app'),
      );
    }
